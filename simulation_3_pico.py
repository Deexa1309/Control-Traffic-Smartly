...
HOST = '192.168.80.136'  # Replace with the microcontroller's IP address if needed
PORT = 80
...
defaultRed = 150
defaultYellow = 5
defaultGreen = 10 ## update this value
defaultMinimum = 5 ## update this value
defaultMaximum = 60
...
def select_next_green(visited_lanes):
    ...
    for direction, count in vehicle_counts.items():
        ...
        if count > max_vehicles and direction_index not in visited_lanes:
            max_vehicles = count
...
def generateVehicles():
    while(True):
        ...
        direction_number = 0
        # Adjusted probabilities to favor left and right lanes
        a = [500, 800, 900, 1000]  # Increased probability for left and right lanes
        if(temp < a[0]):
...
class Main:
    ...
    # background = pygame.image.load('new_simulation/Code/YOLO/darkflow/images/mod_int.png')
